...
;; also defines a txtcite citation type [txtcite:&cite#NN] to open
;; text files associated with the bib entry (via org-open-file) and
;; position the buffer on line NN.
...
;; modifications to ol-bibtex.el for my workflow, add (:file . "Local
;; File") to org-bibtex-fields.  (org-bibtex-create-and-exit): add a
;; less interactive alternative to org-bibtex-create which won't get
;; in your way.

(require 'bbc-gist-notes)
;; see bbc-gist-notes.org.  Convert a publicly available listing of
;; m3u8 urls to live bbc radio streams to an org-file with audio links
;; which can be opened by org-media-note.
