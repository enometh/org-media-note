...
(defconst org-media-note--video-types '("avi" "rmvb" "ogg" "ogv" "mp4" "mkv" "mov" "webm" "flv" "ts" "mpg" "wmv"))
(defconst org-media-note--audio-types '("flac" "mp3" "wav" "m4a" "aac" "opus" "wma"))
...
;; ;madhu 230421
(defun org-media-note--org-citation-from-bibtex-completion-notes (file)
  "Return the citation key from the current bibtex-completion-notes file.
if bibtex-completion-notes-path is a directory then notes are stored
in individual files without a CUSTOM_ID property. The \"link\" is in the
name of the file."
...
(defun org-media-note--current-citation-key ()
  ...
  (or (org-entry-get (point) org-media-note-ref-key-field org-media-note-use-inheritance)
      ...
      (org-media-note--org-citation-from-bibtex-completion-notes buffer-file-name)
...
(defun org-media-note-ref-cite-p ()
  ...
  ;; org-media-note--current-org-ref-citation-key again.
...
(defun org-media-note--link-base-file (file-path)
  ...
  (or (and org-media-note-use-refcite-first
	   (concat (if org-media-note-use-org-ref
		       "&" ; default to org-ref-cite-insert-version == 3
		     )
		   (org-media-note--current-citation-key)))
      (if (org-media-note--online-video-p file-path)
	  file-path
...
(defun org-media-note-seek-to-playback-position (&optional _arg)
  ...
  (save-excursion (beginning-of-line)
		  (re-search-forward "[0-9]+:[0-9]\\{2\\}:[0-9]\\{2\\}")
